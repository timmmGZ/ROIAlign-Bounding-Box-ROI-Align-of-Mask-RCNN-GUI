...
public class Convolution {
	...
	public double[][] createDrawing(double[][] inputs) {
		return matrixSum(filters.stream().map(f -> convolution(inputs, f, 1, true, true)).collect(Collectors.toList()));
	}
	...
	public double[][] convolution(double[][] inputs, double[][] filter, int stride, boolean padding, boolean relu) {
		...
		for (int y = 0; y < output.length; y++)
			for (int x = 0; x < output[0].length; x++)
				max = (output[y][x] = relu ? Math.max(0, convolveOnePixel(inputs, filter, x * stride, y * stride))
						: convolveOnePixel(inputs, filter, x * stride, y * stride)) > max ? output[y][x] : max;
		double scale = max;
		return Arrays.stream(output).map(a -> Arrays.stream(a).map(d -> d / scale).toArray()).toArray(double[][]::new);
	}
